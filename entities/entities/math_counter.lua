...
function ENT:AcceptInput( name, activator, caller, data )
	...
	if name == "add" then
		...
		-- puzzle manager
		if (self.requiredPuzzles && fNewValue <= self.m_flMax) then
			PrintMessage(HUD_PRINTTALK, "[Puzzle Manager] Completed "..fNewValue.."/"..self.requiredPuzzles.." required puzzles.")
...
function ENT:SetOutValue(fNewValue, pActivator, bNoOutput)
	...
	if !bNoOutput and self.Outputs and self.Outputs.OutValue then
		for k,v in pairs(self.Outputs.OutValue) do
			v.param = self.m_OutValue
		end
